...
const BuyerProjectList = () => {
                ...
                for (let i = 0; i < totalProducts_ZJP; i++) {
                    try {
                        ...
                        productList_ZJP.push({
                            ...
                            active: product_ZJP.active,
                            category: product_ZJP.category,
                        });
                    } catch (error) {
                        console.error(`获取索引为 ${i} 的商品信息时出错:`, error);
                    }
                }
    ...
    const handleFilter_ZJP = () => {
        ...
        if (minPriceFilter_ZJP) {
            filtered_ZJP = filtered_ZJP.filter(product_ZJP => product_ZJP.price >= parseInt(minPriceFilter_ZJP));
        }
        if (maxPriceFilter_ZJP) {
            filtered_ZJP = filtered_ZJP.filter(product_ZJP => product_ZJP.price <= parseInt(maxPriceFilter_ZJP));
        }
        if (categoryFilter_ZJP) {
            filtered_ZJP = filtered_ZJP.filter(product_ZJP => product_ZJP.category === categoryFilter_ZJP);
        }
                ...
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: '16px', justifyContent: 'left' }}>
                    {displayedProducts_ZJP.map((product_ZJP, index_ZJP) => (
                        <Card
                            ...
                            hoverable
                            style={{ width: 240 }}
                            cover={<img alt="example" style={{ width: '100%', height: '300px' }} src={"https://gateway.pinata.cloud/ipfs/" + product_ZJP.image} />}
                            onClick={() => {
                                navigate_ZJP(`/apib/buyerProductInfo/${product_ZJP.productId}`)
                            }}
                        >
